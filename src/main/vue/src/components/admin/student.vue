...
<el-button type="primary" @click="adds()">添加学生</el-button>
  </div>
    ...
    <!--
         ...
         <el-form-item label="照片">
           <el-upload
               ...
               action="http://localhost:8088/img?type=student"
               :show-file-list="false"
               :on-success="handleAvatarSuccess"
               :before-upload="beforeAvatarUpload">

             <img v-if="student.imgSrc" :src="'http://localhost:8088/download?name=' + student.imgSrc" class="avatar">
    ...
    <el-form-item label="班级">
      <el-input v-model="student.classes" autocomplete="off"></el-input>
    </el-form-item>
    ...
    <el-button type="primary" @click="add()" :loading="loading">确 定</el-button>
     ...
     <!-- 更改学生提示框 -->
      ...
      <el-dialog title="更改学生" :visible.sync="dialogFormchangeVisible">
    ...
    <el-form-item label="密码">
      <el-input v-model="student.pwd" autocomplete="off" type="password"></el-input>
    </el-form-item>
...
export default{
     ...
     methods: {
       ...
       req() {
         axios.get("student/findAll").then((res) => {
       ...
       del(row) {
         this.$confirm('删除学生是否继续?', '提示', {
           confirmButtonText: '确定',
           cancelButtonText: '取消',
           type: 'warning'
         }).then(() => {

           axios.post('student/DelOne',row).then((res) => {

             console.log(res)
             console.log("123213")
             this.req()

             axios.post("http://127.0.0.1:2020/delete_face?id=" + row.id).then(res=>{
               if(res.data === 200) this.$message.success("人脸库删除成功")
             })
       ...
       add() {
          ...
          */
           ...
           //加入到学生数据库
           axios.post('student/new_add', param).then(res => {
             this.$message.success("添加成功")


             console.log("12312313")
             //新建账户
             axios.post("money/add_bank/" + this.student.id).then(res=>{
               console.log(res)
             })
